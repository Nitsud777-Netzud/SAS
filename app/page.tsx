...
export default function Home() {
  return (
    <main>
      <header className="container flex items-center justify-between border-b-2 border-black py-5">
        <a href="#top" className="display text-2xl">S.A.S.</a>
        <nav className="hidden gap-7 text-xs font-black tracking-widest md:flex">
          {nav.map((item) => <a key={item} href={`#${item.toLowerCase().replaceAll(" ", "-")}`} className="hover:underline">{item}</a>)}
        </nav>
        <a href="#get-involved" className="bg-black px-4 py-2 text-xs font-black tracking-widest text-white">JOIN US ↗</a>
      </header>
      ...
      <section id="top" className="grid-bg border-b-2 border-black py-20 md:py-28">
        <div className="container">
          <p className="mb-6 text-xs font-black tracking-[.2em]">YOUTH-LED • INLAND EMPIRE • CALIFORNIA</p>
          <h1 className="display max-w-5xl text-[clamp(4rem,11vw,9.5rem)] leading-[.82] uppercase">Students<br/><span className="bg-[var(--accent)] px-2">Against</span><br/>Surveillance.</h1>
          <div className="mt-12 grid gap-8 md:grid-cols-[1fr_360px] md:items-end">
            ...
            <div className="flex flex-col gap-3 sm:flex-row md:flex-col">
              <a href="#the-issue" className="border-2 border-black px-5 py-4 text-center text-sm font-black tracking-widest hover:bg-black hover:text-white">LEARN ABOUT THE ISSUE ↘</a>
              <a href="#get-involved" className="bg-black px-5 py-4 text-center text-sm font-black tracking-widest text-white hover:bg-[var(--accent)] hover:text-black">GET INVOLVED ↗</a>
            </div>
          </div>
        </div>
      </section>

      <section id="about" className="container border-b-2 border-black py-20 md:py-28">
        <div className="grid gap-12 md:grid-cols-[.7fr_1.3fr]">
          <p className="text-xs font-black tracking-[.2em]">01 / ABOUT S.A.S.</p>
          <div>
      ...
      <section id="the-issue" className="border-b-2 border-black bg-black py-20 text-white md:py-28">
        <div className="container">
      ...
      <section id="get-involved" className="border-b-2 border-black py-20 md:py-28">
        <div className="container">
          <div className="grid gap-12 md:grid-cols-[1fr_1fr]">
            <div><p className="mb-5 text-xs font-black tracking-[.2em]">03 / GET INVOLVED</p><h2 className="display text-6xl uppercase leading-[.9] md:text-8xl">Don't just<br/>watch.</h2></div>
            <div className="flex flex-col justify-between"><p className="text-xl leading-8">S.A.S. is being built by students. Volunteer, help with research, organize with other students, or help us build the next chapter.</p><a href="#" className="mt-10 inline-block self-start bg-[var(--accent)] px-7 py-5 text-sm font-black tracking-widest hover:bg-black hover:text-white">VOLUNTEER WITH S.A.S. ↗</a></div>
          </div>
        </div>
      </section>

      <section id="updates" className="container py-20 md:py-28">
        <div className="flex items-end justify-between border-b-2 border-black pb-5"><div><p className="mb-4 text-xs font-black tracking-[.2em]">04 / UPDATES</p><h2 className="display text-5xl uppercase md:text-7xl">What's happening</h2></div><a href="#" className="hidden text-xs font-black tracking-widest underline sm:block">ALL UPDATES ↗</a></div>
